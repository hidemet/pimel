@props([
    'navBgClass' => 'bg-body-public-default', // Default per navbar pubblica
])

<header class="sticky-top">
    <nav class="navbar navbar-expand-md py-3 px-5 {{ $navBgClass }} shadow-sm">
        {{-- 1. LOGO (a sinistra per default) --}}
                ...
                <li class="nav-item mt-2 mt-md-0 ms-md-2">
                    @guest
                        <a class="nav-link text-custom-navbar p-2" href="{{ route('login') }}" title="Accedi o Registrati">
                            <span class="material-symbols-outlined align-middle fs-4">person</span>
                        </a>
                    @else
                        <div class="dropdown">
                            <a class="nav-link dropdown-toggle d-flex align-items-center p-2" href="#"
                                id="userDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                                <span class="material-symbols-outlined fs-4 me-md-1">account_circle</span>
                                <span
                                    class="d-none d-md-inline-block small">{{ Str::limit(Auth::user()->name, 15) }}</span>
                            </a>
